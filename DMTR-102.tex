...
demonstrate that the key components can interoperate successfully.\\
~\\
It will specifically demonstrate that:
...
\item
  The raw data images can be ingested into a Butler (eventually by the
  DBB along with a subset of image and provenance metadata)
\item
...
This iteration of the test doesn't include the CCOB data, but the AuxTel
data was the considered the closest to a CCOB data devices available for
quite some time. ~ More iterations of this test with more automation is
required without humans involved in the first 1/2 of 2020 with the
spectrograph working at the summit.~ ~
...
system (spectrograph, comcam, lsstcam, CCOB... others) as to where the
...
scientists AFTER it was ingested not before. ~ ~There was no CCOB or DBB
...
data is then transferred to NCSA.~ ~
      ...
      \begin{minipage}[t]{13cm}{\footnotesize
      An image file was created by the AT-archiver software machine and placed
into the~ transfer directory. ~ The file directory on the AT-archiver is
/data/export/2019-06-11. ~ It had 1 image file(s) created. ~ ~The image
has proper headers generated by the header service. ~
